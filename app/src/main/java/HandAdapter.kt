...
class HandAdapter(private val onCardSelectionChanged: (List<Card>) -> Unit) : 
    ...
    inner class CardViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(card: Card, isSelected: Boolean) {
            ...
            // 根据选中状态设置垂直平移
            itemView.translationY = if (isSelected) -30f else 0f // 向上平移20f
            
            // 设置点击事件
            itemView.setOnClickListener {
                val card = getItem(adapterPosition) // 获取当前位置的卡牌
                val isCurrentlySelected = selectedCards.contains(card) // 判断当前是否选中

                if (isCurrentlySelected) {
                    selectedCards.remove(card)
                    itemView.translationY = 0f // 恢复原始位置
                } else {
                    selectedCards.add(card)
                    itemView.translationY = -30f // 向上平移
                }
                // 我们直接更新了translationY，所以不需要notifyItemChanged来触发rebind
